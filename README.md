...
| # | 漏洞 | 风险等级 | 入口 | 摘要 |
|---|------|---------|------|------|
| 1 | 0元购（新购） | 严重 | `/v1/cart/checkout`、`/cart/settle`、`/cart?action=viewcart&statuscart=checkout` | 结算接口信任客户端传入的 `resource_percent_value` 参数，传 0 可将订单金额归零 |
| 2 | 0元续费 | 严重 | `/servicedetail?action=renew`、`/v1/hosts/*/renew`、`/v1/products/*/renew` | 续费接口信任客户端传入的 `resource_handling` 参数，传 0 可将续费金额归零 |
| 3 | 0元升级 | 严重 | `/upgrade/*Checkout`、`/v1/hosts/*/actions/upgradeconfig`、`/v1/hosts/*/actions/upgrade` | 升级接口信任客户端传入的 `resource_percent_value` 参数，传 0 可将升级金额归零 |
...
- 攻击者可通过续费接口构造折扣参数为 0 的请求实现 0 元续费，到期时间延长
- 攻击者可通过升级接口构造折扣参数为 0 的请求实现 0 元升级，配置提升
- 折扣参数可随查询串、表单或 JSON 请求体提交，WAF 等外层防护难以识别
- v1 接口和面板接口均可被利用，仅修复一个入口无法根治
- 0元续费可绕过管理员手动暂停，被暂停的产品可自动解除暂停
...
1. **拦截新购0元购** — 结算接口的折扣参数（`resource_percent_value` 等）
2. **拦截0元续费** — 续费接口的折扣参数（`resource_handling` 等）
3. **拦截0元升级** — 升级接口的折扣参数（`resource_percent_value` 等）
4. **强制图形验证码校验**（发送短信/邮件时必须通过图形验证码）
5. **发送频率限制**（同一手机号/邮箱，60秒内最多N次）
6. **改密安全加固**（拦截未携带旧密码的请求）
7. **开放重定向防护**（redirect_url 仅允许站内域名）
8. **注册验证码强制**（系统开启图形验证码时，注册必须通过校验）
9. 命中即返回 HTTP 400 + 随机嘲讽文案，不再进入结算/续费/升级逻辑
...
| 入口 | 路径 | 拦截 |
|------|------|------|
| 面板续费接口 | `/servicedetail?action=renew` | ✅ |
| v1 续费接口 | `/v1/hosts/*/renew`、`/v1/products/*/renew` | ✅ |

**升级：**
| 入口 | 路径 | 拦截 |
|------|------|------|
| 面板升级接口 | `/upgrade/*Checkout` | ✅ |
| v1 升级接口 | `/v1/hosts/*/actions/upgradeconfig`、`/v1/hosts/*/actions/upgrade` | ✅ |
...
- C2F4 — [GitHub](https://github.com/AgFox-super/czeidc)

## 友链

- [帷云](https://www.vyidc.top)
- [锚点云](https://www.mdidc.net)
- [沧舟云](https://idc.mcedm.top)
